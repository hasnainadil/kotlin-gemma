...
import com.google.ai.edge.gallery.ui.functioncalling.FunctionCallingDestination
import com.google.ai.edge.gallery.ui.functioncalling.FunctionCallingScreen
...
fun GalleryNavHost(
  ...
  NavHost(
    ...
    composable(
      ...
      getModelFromNavigationParam(backStackEntry, TASK_LLM_ASK_IMAGE)?.let { defaultModel ->
        ...
        LlmAskImageScreen(
          ...
          modelManagerViewModel = modelManagerViewModel,
          navigateUp = { navController.navigateUp() },
          navigateToDiseaseScan = { 
            navController.navigate("${DiseaseScanningDestination.route}/${defaultModel.name}")
          },
        )
      }
    }
    ...
    // Function calling.
    composable(
      route = "${FunctionCallingDestination.route}/{modelName}",
      arguments = listOf(navArgument("modelName") { type = NavType.StringType }),
      enterTransition = { slideEnter() },
      exitTransition = { slideExit() },
    ) { backStackEntry ->
      getModelFromNavigationParam(backStackEntry, TASK_LLM_FUNCTION_CALLING)?.let { defaultModel ->
        modelManagerViewModel.selectModel(defaultModel)
    ...
    // Disease scanning.
    composable(
      route = "${DiseaseScanningDestination.route}/{modelName}",
      arguments = listOf(navArgument("modelName") { type = NavType.StringType }),
      enterTransition = { slideEnter() },
      exitTransition = { slideExit() },
    ) { backStackEntry ->
      val viewModel: DiseaseScanningViewModel = hiltViewModel()

      getModelFromNavigationParam(backStackEntry, TASK_LLM_DISEASE_SCANNING)?.let { defaultModel ->
        modelManagerViewModel.selectModel(defaultModel)

        DiseaseScanningScreen(
          viewModel = viewModel,
          modelManagerViewModel = modelManagerViewModel,
          navigateUp = { navController.navigateUp() },
        )
